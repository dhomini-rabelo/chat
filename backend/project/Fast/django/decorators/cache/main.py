    ...
    def decorator_function(view_function):
        def wrapper_function(*args, **kwargs):
            ...
            if cache.get(request.path) is None:
                ...
                cache.set(request.path, response.content, cache_timeout)
                return response
...
def super_cache_page(global_cache_timeout: int, browser_cache_timeout: int):
    def decorator_function(view_function):
        ...
        def wrapper_function(*args, **kwargs):
            ...
            if cache.get(request.path) is None:
                response = view_function(*args, **kwargs)
                cache.set(request.path, response.content, global_cache_timeout)
                return response
            return HttpResponse(cache.get(request.path))
        return wrapper_function
    return decorator_function
...
def static_page(view_function):
    ...
    def wrapper_function(*args, **kwargs):
        ...
        if cache.get(request.path) is None:
            response = view_function(*args, **kwargs)
            cache.set(request.path, response.content, None)
            return response
        return HttpResponse(cache.get(request.path))
    return wrapper_function
